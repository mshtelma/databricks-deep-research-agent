...
from deep_research_agent.core.section_models import (
...
class EnhancedWorkflowNodes:
    ...
    def coordinator_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        from deep_research_agent.core.contract_node_handler import execute_contract_agent_with_circuit_breaker
    ...
    def researcher_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not (plan and getattr(plan, 'report_template', None)):
                ...
                from deep_research_agent.core.report_styles import STYLE_CONFIGS, ReportStyle
            ...
            if plan and getattr(plan, 'report_template', None):
                ...
                from deep_research_agent.core.debug_utils import dump_section_state, validate_section_id_consistency
                ...
                if current_step:
                    ...
                    if section_spec:
                        ...
                        try:
                            ...
                            logger.info(f"Completed step {current_step.step_id} for section '{section_spec.title}'")
                            
                        except Exception as e:
                            error_msg = f"Failed to research section {section_spec.title}: {e}"
                            logger.error(error_msg)
                            
                            # Special handling for IP blocking errors (403)
                            error_str = str(e)
                            if "403" in error_str and ("IP ACL" in error_str or "IP address" in error_str):
                                logger.warning(f"IP blocked by Databricks ACL - marking step as completed with degraded content")
                                # Create a degraded result with available data
                                existing_observations = state.get("observations", [])
                                failure_result = SectionResearchResult(
                                    synthesis=f"Content generation limited due to IP restrictions. Section: {section_spec.title}. Using available research data.",
                                    observations=tuple(existing_observations[-5:]) if existing_observations else tuple(),  # Use last 5 observations
                                    confidence=0.2,
                                    metadata={
                                ...
                                section_results[section_key] = failure_result
                                state["section_research_results"] = section_results
                                self._update_section_title_mapping(state, plan, {section_key: failure_result})
                                
                                # Mark as completed with degraded content rather than failed
                                if plan:
                                    plan.mark_step_completed(
                                        current_step.step_id,
                                ...
                                # Normal error handling for non-403 errors
                                ...
                                from deep_research_agent.core.routing_policy import increment_failed_attempts, should_skip_failed_step
                                ...
                                # Store error result with same structure
                                section_key = (
                                    id_gen.PlanIDGenerator.normalize_id(section_spec.id)
                                    if isinstance(section_spec.id, str)
                                    else section_spec.id
                                ...
                                logger.warning(f"Failed step {current_step.step_id} for section '{section_spec.title}'")
                    else:
                        # Handle non-section steps (e.g., processing, synthesis)
                        from deep_research_agent.core.plan_models import StepType
                        if current_step.step_type in (StepType.PROCESSING, StepType.SYNTHESIS):
                            ...
                            error_msg = (
                                f"No section found for step {current_step.step_id} "
                                f"(normalized={target_section_id})"
                            )
                            logger.error(error_msg)
                            ...
                            state["current_step"] = None  # Clear invalid step
            ...
            if isinstance(result, dict) and "section_research_results" in result:
                ...
                from deep_research_agent.core.debug_utils import validate_section_research_state
            ...
            if isinstance(result, dict) and "section_research_results" in result:
                ...
                from deep_research_agent.core.debug_utils import dump_section_state, validate_section_research_state
    ...
    def fact_checker_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            from deep_research_agent.core.routing_policy import ProgressMetrics
    ...
    def _process_search_results(self, raw_results: List[Any]) -> List[SearchResult]:
        ...
        from deep_research_agent.core.types import SearchResult
