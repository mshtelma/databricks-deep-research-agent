...
from .constants import AGENT_CONFIG_FILENAME, CONFIG_SEARCH_ORDER
from .core.error_handling import handle_agent_error, create_error_response
            ...
            if not yaml_path:
                ...
                if not yaml_path:
                    ...
                    if default_path.exists():
                        ...
                        # In tests, gracefully skip agent initialization so we can mock data
                        if env_context == "test environment":
                            logger.warning(
                                "Configuration not found in test environment; skipping DatabricksCompatibleAgent initialization"
                            )
                            self.agent = None
                            self.config = None
                            return
