...
def extract_numbers_with_context(text: str) -> list[tuple[float, str, bool, bool]]:
    ...
    # Pre-normalize: insert space after comma when it separates two independent
    # decimal numbers in array contexts (e.g., "44.00,231.52" → "44.00, 231.52").
    # This prevents the thousands-comma regex from treating the comma as a
    # thousands separator.  Pattern matches: decimal-number COMMA digit — only
    # fires when a decimal point precedes the comma, which never happens in
    # legitimate thousands separators like "1,234".
