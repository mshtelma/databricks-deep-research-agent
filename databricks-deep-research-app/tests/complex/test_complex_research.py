...
class TestComplexResearch:
    ...
    async def test_multi_entity_comparison(
        ...
        assert result.state.final_report, "Should produce a comparison report"
        assert len(result.state.final_report) > 500, "Three-entity comparison should be substantive"
    ...
    async def test_deep_dive_with_follow_ups(
        ...
        assert any(
            term in followup_lower for term in ("mrna", "messenger rna")
        ), "Follow-up should reference mRNA"
        assert any(
            term in followup_lower for term in ("cancer", "oncology", "tumor")
        ), "Follow-up should reference cancer"
        assert any(
            term in followup_lower for term in ("clinical", "trial", "study", "treatment")
        ), "Follow-up should discuss trials or treatment"
    ...
    async def test_comprehensive_citation_verification(
        ...
        assert any(
            term in report_lower for term in ("gw", "gigawatt", "tw", "terawatt", "mw")
        ), "Report should cite capacity units"
        assert any(
            term in report_lower for term in ("%", "percent", "billion", "trillion")
        ), "Report should cite scale metrics"
        ...
        # Verify sources
        assert len(result.state.sources) >= 2, "Renewable-energy statistics should use multiple sources"
