...
async def test_deep_research_initial_query(
    ...
    assert len(result.state.final_report) > 300, "Report should have meaningful content"

    # Verify sources were found
    assert len(result.state.sources) >= 2, "Deep research should use multiple sources"

    # Verify research steps executed
    assert result.steps_executed >= 1, "Should have executed at least one research step"
    ...
    assert result.state.query_classification.complexity, "Query should be classified"
    ...
    assert len(result.state.claims) >= 3, "Should have extracted multiple claims"
    ...
    assert len(result.state.evidence_pool) >= 1, "Should have evidence spans"
...
async def test_followup_with_conversation_history(
    ...
    assert len(followup_result.state.final_report) > 200, "Follow-up report should have content"
    ...
    assert any(
        term in report_lower for term in ("quantum", "entanglement", "entangle")
    ), "Follow-up should preserve the quantum context"
    assert any(
        term in report_lower for term in ("computer", "computing", "qubit", "gate")
    ), "Follow-up should discuss quantum-computing applications"
...
async def test_streaming_research_events(
    ...
    assert len(events) >= 5, "Streaming flow should emit a meaningful event sequence"
    ...
    assert len(full_synthesis) >= 50, "Should have streamed substantive synthesis content"
...
async def test_research_with_complex_query(
    ...
    assert len(result.state.final_report) > 400, "Complex query should produce detailed report"
