...
class TestWebCrawlToolExecutionWithUrl:
    ...
    async def test_crawl_returns_data(self) -> None:
        ...
        assert result.data["content_length"] == len("This is the page content.")
        assert result.sources is not None
        assert result.sources[0]["content_length"] == result.data["content_length"]
...
class TestWebCrawlToolExecutionWithIndex:
    ...
    @pytest.mark.asyncio
    async def test_crawl_uses_registry_title_fallback(self) -> None:
        """Registry title should be used when the crawler result omits one."""
        mock_crawler = create_mock_crawler(results=[
            CrawlResult(
                url="https://example.com/page",
                title=None,
        ...
        assert result.success
        assert "Registered Page" in result.content
        assert result.sources is not None
        assert result.sources[0]["title"] == "Registered Page"

    @pytest.mark.asyncio
    async def test_crawl_reports_large_content_length(self) -> None:
        """Tool output should preserve the transformed content length contract."""
        large_content = "x" * MAX_CONTENT_LENGTH
        mock_crawler = create_mock_crawler(results=[
            CrawlResult(
                url="https://example.com/page",
                title="Large Page",
        ...
        result = await tool.execute({"url": "https://example.com/page"}, create_test_context())

        assert result.success
        assert result.data is not None
        assert result.data["content_length"] == MAX_CONTENT_LENGTH
