...
class ChatLogger:
    ...
    def __init__(self, log_file_path: str = "/tmp/databricks-app-chat.log"):
        ...
        if not any(isinstance(h, logging.StreamHandler) for h in self.chat_logger.handlers):
            ...
            console_handler.setFormatter(logging.Formatter("%(asctime)s - CHAT - %(message)s"))
            self.chat_logger.addHandler(console_handler)

    def log_request(
        self, request_id: str, user_info: Dict[str, Any], prompt: str, messages_history: Optional[List[Dict]] = None
    ) -> Dict[str, Any]:
    ...
    def log_streaming_event(
        self, request_id: str, event_type: str, phase: Optional[str] = None, progress: Optional[float] = None
    ):
    ...
    def log_error(
        self, request_id: str, error_message: str, error_type: str = "unknown", traceback_str: Optional[str] = None
    ):
    ...
    def get_stats(self, last_hours: int = 24) -> Dict[str, Any]:
        ...
        return {"message": "Statistics calculation not yet implemented", "last_hours": last_hours}
...
class ChatRequestContext:
    ...
    def __enter__(self):
        ...
        self.logger.log_request(self.request_id, self.user_info, self.prompt, self.messages)
        return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_val:
            ...
            self.logger.log_response(
                self.request_id, self.response_content, elapsed_ms, self.stream_events_count, error=str(exc_val)
            )
        else:
            # Log success case
            self.logger.log_response(self.request_id, self.response_content, elapsed_ms, self.stream_events_count)
    ...
    def add_stream_event(self, event_type: str, content: str = "", metadata: Dict = None):
        ...
        if event_type == "research_update" and metadata:
            self.logger.log_streaming_event(
                self.request_id, event_type, phase=metadata.get("phase"), progress=metadata.get("progress_percentage")
            )
