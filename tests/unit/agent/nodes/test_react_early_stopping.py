...
class TestInfoGainTracking:
    ...
    @pytest.mark.asyncio
    async def test_record_first_tool_call(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_consecutive_low_gain_tracking(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_consecutive_reset_on_gain(self) -> None:
...
class TestEarlyStopping:
    ...
    @pytest.mark.asyncio
    async def test_no_stop_before_min_calls(self) -> None:
        ...
        should_stop, reason = await state.should_stop_early(min_calls=5)
        assert should_stop is False

    @pytest.mark.asyncio
    async def test_no_stop_without_min_sources(self) -> None:
        ...
        should_stop, reason = await state.should_stop_early(min_sources=3)
        assert should_stop is False

    @pytest.mark.asyncio
    async def test_stop_on_diminishing_returns(self) -> None:
        ...
        should_stop, reason = await state.should_stop_early(
    ...
    @pytest.mark.asyncio
    async def test_stop_on_high_coverage(self) -> None:
        ...
        should_stop, reason = await state.should_stop_early(min_calls=5, min_sources=3)
        assert should_stop is True
        assert "high_coverage" in reason

    @pytest.mark.asyncio
    async def test_no_stop_with_high_recent_gain(self) -> None:
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_typical_research_session(self) -> None:
        ...
        should_stop, _ = await state.should_stop_early(min_calls=5, min_sources=3)
